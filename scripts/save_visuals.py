...
def main() -> None:
    parser = argparse.ArgumentParser(description="Save visualization HTML files.")
    ...
    for name, fig in figures.items():
        filename = f"viz_{name}_plant{args.plant}_{timestamp}.html"
        save_figure(fig, os.path.join(args.output, filename))
